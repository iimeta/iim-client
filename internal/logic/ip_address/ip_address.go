...
func init() {
	service.RegisterIpAddress(New())
}

func New() service.IIpAddress {
	return &sIpAddress{}
}
...
func (s *sIpAddress) FindAddress(ctx context.Context, ip string) (string, error) {
	...
	err := util.HttpGet(ctx, url, nil, params, &data)
	if err != nil {
	...
	val := strings.Join(util.Unique(arr), " ")
